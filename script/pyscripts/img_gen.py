...
def main(extra_args):
    ...
    for filename in file_list:
        ...
        if not entries:
            print(f"{filename}: No image tags found")
            continue
        print(f"{filename}: {len(entries)} tags found")
        replace_tags(filename, entries)
        errCount += diff_before_continue(filename)
...
def replace_tags(filename, entries):
    ...
    with open(f"{filename}.bak", 'r') as _file:
        for line in _file:
            line_num += 1
...
def diff_before_continue(filename, skip_diff):
    if not skip_diff:
        os.sys(f"diff {filename} {filename}.bak")
        result = input("OK to cleanup backups? [y/N]")
...
if __name__ == "__main__":
    try:
        tag_index = sys.args.index("--tag")
        tag = sys.args[tag_index + 1]
    except IndexError:
        print("Syntax error with custom tag.")
        sys.exit(1) 
    except ValueError:
        tag = "[img]"
    extra_args["tag"] = tag
