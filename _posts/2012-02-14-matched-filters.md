...
<a href="http://kevinfronczak.com/documents/noiseandsignal.png"><img class="aligncenter" src="{{ site.baseurl }}/assets/noiseandsignal.png" alt="" width="500" /></a>
...
<a href="http://kevinfronczak.com/documents/lpf.jpg"><img class="aligncenter" src="{{ site.baseurl }}/assets/lpf.jpg" alt="" width="400/" /></a>
...
<a href="http://kevinfronczak.com/documents/lpf_freqeuncy_response.png"><img class="aligncenter" src="{{ site.baseurl }}/assets/lpf_freqeuncy_response.png" alt="" width="600" /></a>
...
<a href="http://kevinfronczak.com/documents/matchedFilterSystem.png"><img class="aligncenter" src="{{ site.baseurl }}/assets/matchedFilterSystem.png" alt="" width="600" /></a>
...
As discussed before, we know that the Power Spectral Density, [latex size="1"]S_{N_{i}}(\omega) = \frac{N_{0}}{2}$$.  As you might expect, the output is simply going to be scaled by the magnitude of the Transfer Function so [latex size="1"]S_{N_{0}}(\omega) = S_{N_{0}}(\omega)|H(\j\omega)|^{2} = \frac{N_{0}}{2}\frac{1}{1+\omega^{2}R^{2}C^{2}}$$.
To extract the average power from the Power Spectral Density, we simply multiply by [latex size="1"]\frac{1}{2\pi}$$ and integrate from $$-\infty$$ to $$\infty$$.  This yields [latex size="2"]P_{N_{0}} = \frac{1}{2\pi}\int_{-\infty}^{\infty} S_{N_{0}}(\omega)\,d\omega= \frac{1}{2\pi}\int_{-\infty}^{\infty} \frac{N_{0}/2}{1+\omega^{2}R^{2}C^{2}},d\omega$$.  Now this certainly appears to be a rather tedious integral to calculate.  However, if you recognize that [latex size="1"]\int \frac{dx}{1+ \alpha^{2}x^{2}} = \frac{1}{\alpha} = \tan^{-1}(\alpha x)$$ and that we can say $$\alpha = RC$$ then our integral just became a whole lot simpler!  Solving the integral leaves us with [latex size="1"]P_{N_{0}} = \left.\frac{N_{0}}{4\pi}\frac{1}{RC}\tan^{-1}(RC\omega_{0})\right|_{-\infty}^{\infty} = \frac{N_{0}}{4\pi RC}\left(\frac{\pi}{2}-\frac{-\pi}{2}\right) $$
Therefore, the average noise power at the output is given by: [latex size="1"]\frac{N_{0}}{4RC}$$.
Now we can calculate $$SNR_{o}$$.  Since this is just equal to [latex size="1"]\frac{Signal Power}{Noise Power}$$ we say [latex size="2"]SNR_{o} = \frac{\frac{A^{2}}{2(1+ \omega^{2} R^{2}C^{2})}}{\frac{N_0}{4 RC}}$$[latex size="2"]=\frac{A^{2}}{2(1+ \omega^{2} R^{2}C^{2})}\cdot\frac{4 RC}{N_0}$$[latex size="2"]= \frac{2 A^2}{N_0}\cdot\frac{RC}{1+ \omega^2 R^2 C^2}$$.
Now our goal is to design our filter to maximize $$SNR$$ so we need to take the derivative with respect to $$RC$$ and set that derivative equal to zero.  To make our calculations a bit simpler, let's set $$x = RC$$ so that [latex size="1"]SNR_0 = \frac{2 A^2}{N_0}\cdot\frac{x}{1+ \omega_{0}^{2} x^{2}}$$.  Now, we take the derivative:
[latex size="2"]\frac{d(SNR_0)}{dx} = \frac{2 A^2}{N_0}\cdot\frac{1+\omega_{0}^{2} x^{2} - 2 \omega_{0}^{2} x^{2}}{(1+\omega_{0}^{2} x^{2})^{2}} = 0$$.  Now we just need to solve for x.
[latex size="1"]1+\omega_{0}^{2} x^{2} - 2 \omega_{0}^{2} x^{2} = 0 \Rightarrow \omega_{0}^{2} x^{2} = 1 \therefore RC = \frac{1}{\omega_{0}}$$.
Whoa! This result looks awfully familiar, doesn't it?  Remember when we found that [latex size="1"]\omega_{3dB} = \frac{1}{RC}$$?  Well, we just found that in order to maximize our $$SNR$$ we should make the bandwidth of our filter equivalent to the 3dB bandwidth!  Pretty neat, huh?!
In that case, what is the best $$SNR$$ we can achieve with this configuration?  Well, let's solve for it! [latex size="2"]SNR_{max} = \frac{2A^2}{N_0}\cdot\frac{1/ \omega_0}{1+\omega_{0}^{2}\cdot \frac{1}{\omega_{0}^{2}}} = \frac{2A^{2}}{N_0}\cdot\frac{1/ \omega_0}{2} \therefore SNR_{max} = \frac{A^2}{N_0 \omega_0}$$.
